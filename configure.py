...
def main(args):
    ...
    with build.options(source_dir="src", macros=macros,
            deps={
                (build.deps.cpuinfo, build.deps.fxdiv, build.deps.fp16): any,
        ...
        if backend == "x86_64":
            ...
            arch_nnpack_objects = [
                # Transformations
                build.cxx("psimd/2d-fourier-8x8.cpp"),
                build.cxx("psimd/2d-fourier-16x16.cpp"),
                build.cxx("psimd/2d-winograd-8x8-3x3.cpp"),
                # Tuple GEMM
                build.cxx("psimd/blas/s4gemm.cpp"),
                build.cxx("psimd/blas/c4gemm-conjb.cpp"),
                build.cxx("psimd/blas/s4c2gemm-conjb.cpp"),
                # Direct convolution
                build.cxx("psimd/blas/conv1x1.cpp"),
                # BLAS microkernels
                build.cxx("psimd/blas/sgemm.cpp"),
            ]
            if not options.inference_only:
                arch_nnpack_objects += [
                    # Tuple GEMM
                    build.cxx("psimd/blas/c4gemm.cpp"),
                    build.cxx("psimd/blas/s4c2gemm.cpp"),
                    build.cxx("psimd/blas/c4gemm-conjb-transc.cpp"),
                    build.cxx("psimd/blas/s4c2gemm-conjb-transc.cpp"),
                ]
            if not options.convolution_only:
                arch_nnpack_objects += [
                    # Activations
                    build.cxx("psimd/relu.cpp"),
                    build.cxx("psimd/softmax.cpp"),
                    # BLAS microkernels
                    build.cxx("psimd/blas/sdotxf.cpp"),
                    build.cxx("psimd/blas/shdotxf.cpp"),
                ]
        ...
        if backend == "x86_64":
            ...
            arch_fft_stub_objects = [
                build.cxx("psimd/fft-aos.cpp"),
                build.cxx("psimd/fft-soa.cpp"),
                build.cxx("psimd/fft-real.cpp"),
                build.cxx("psimd/fft-dualreal.cpp"),
            ]

            if backend == "psimd":
                arch_winograd_stub_objects = [
                    build.cxx("psimd/winograd-f6k3.cpp"),
                ]
            else:
